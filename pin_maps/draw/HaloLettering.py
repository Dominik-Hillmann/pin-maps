raise NotImplementedError('This will be implemented in iteration 2. Code below was to try out, works.')
'''
# Internal modules
...
class OffsetLettering(ImageTransform):
    ...
    def transform(self, heraldry: Image.Image) -> Image.Image:
        ...
        lettering_dims = tuple(val + 4 for val in lettering_dims)
        # halo_gaps = [-1, 1]
        lettering = Image.new('RGBA', lettering_dims, (0, ) * 4)
        lettering_drawing = ImageDraw.Draw(lettering)
        
        lettering_drawing.text((0, 0), self.town_name, self.__background_col, self.__font)
        lettering_drawing.text((4, 0), self.town_name, self.__background_col, self.__font)
        lettering_drawing.text((0, 4), self.town_name, self.__background_col, self.__font)
        lettering_drawing.text((4, 4), self.town_name, self.__background_col, self.__font)
        lettering_drawing.text((2, 2), self.town_name, self.__foreground_col, self.__font)
